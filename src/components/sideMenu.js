...
export default function SideMenu({
  ...
  return (
    <div className='flex'>
      <aside
        className={`px-3 py-5 w-68 z-10 flex-shrink-0 h-screen overflow-y-auto sticky top-0 left-0 flex flex-col items-center justify-center transition-all duration-300 bg-themeColor-faded max-md:absolute ${
          isMenuVisible
      ...
      >
        ...
        <p className='mt-4 text-lg font-bold'>{`${capitalize(
          name
        )} ${capitalize(surname)}`}</p>
        <p className='text-sm font-medium text-themeColor'>
          {role}
        </p>
